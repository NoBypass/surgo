...
`, ts, name, ts2, name2)
	...
	t.Run("Scan only", func(t *testing.T) {
		only := testObject{}
		err := db.Scan(&only, "RETURN (SELECT * FROM test WHERE time_since = $time_since)[0]", testObject{TimeSince: ts})
		assert.NoError(t, err)
		assert.Equal(t, testObject{TimeSince: ts, Name: name}, only)
	})
	t.Run("Scan to slice", func(t *testing.T) {
		var only []testObject
		err := db.Scan(&only, "SELECT * FROM test WHERE time_since = $1", ts)
		assert.NoError(t, err)
		assert.Equal(t, []testObject{{TimeSince: ts, Name: name}}, only)
	})
	...
	t.Run("Scan to single variable", func(t *testing.T) {
		var count int
		err := db.Scan(&count, "RETURN (SELECT count() FROM test GROUP ALL)[0].count")
		assert.NoError(t, err)
